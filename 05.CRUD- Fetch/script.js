...
const getAll = async () => {
    try {
        let response = await fetch("http://localhost:5555/mobs"),
            json = await response.json();

        if (!response.ok) throw { status: response.status, statusText: response.statusText }; //Si existe un error.
    ...
    if (e.target === $form) {
        ...
        if (!e.target.id.value) {
            ...
            try {
                ...
                response = await fetch("http://localhost:5555/mobs", options),
                    json = await response.json();
            ...
            try {
                ...
                response = await fetch(`http://localhost:5555/mobs/${e.target.id.value}`, options),
                    json = await response.json();
